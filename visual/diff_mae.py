...
def diff(year, leadingtime, method="QM"):
    ...
    mymodel_path = f"{base_path}mae_median/model_G_i000008_20240824-212330_with_huber/{year}/lead_time{leadingtime}_whole.npy"
    compare_path = ""

    if method == "QM":
        compare_path = f"/scratch/iu60/xs5813/qm/new_crps/save/mae_median/QM/{year}/lead_time{leadingtime}_whole.npy"
        ...
        compare_path = f"/scratch/iu60/xs5813/cli_metric_result/new_crps/save/climatology/mean_climatology/{year}/window1/mae_median_climat_lead_time_{leadingtime}.npy"
    elif method == "DESRGAN":
        compare_path = f"{base_path}mae_median/model_G_i000006_20240610-011512/{year}/lead_time{leadingtime}_whole.npy"
    else:
        print("输入的方法名称有误，请检查！")
        return
    ...
    # 绘图
    draw(my_model_data, compare_data, lon, lat, title=f"mae difference at leadingtime {leadingtime} compare with {method}")
...
def diff_average(year, leadingtime_start,leadingtime_end, method="QM"):
    ...
    lon = awap_data.lon.values
    base_path = "/scratch/iu60/xs5813/metric_results/"
    model_prefix = f"{base_path}mae_median/model_G_i000008_20240824-212330_with_huber/{year}/lead_time"
    compare_prefix = ""

    if method == "QM":
        compare_prefix = f"/scratch/iu60/xs5813/qm/new_crps/save/mae_median/QM/{year}/lead_time"
    elif method == "climatology":
        compare_prefix = f"/scratch/iu60/xs5813/cli_metric_result/new_crps/save/climatology/mean_climatology/{year}/window1/mae_median_climat_lead_time_"
    elif method == "DESRGAN":
        compare_prefix = f"{base_path}mae_median/model_G_i000006_20240610-011512/{year}/lead_time"
    else:
        print("输入的方法名称有误，请检查！")
        return
    ...
    # 绘图
    draw(my_model_avg, compare_model_avg, lon, lat, title=f"Average mae difference at {year} from leadingtime {leadingtime_start} to {leadingtime_end} compared with {method}")
#diff(2018,1,method="QM")
